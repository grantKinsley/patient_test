    ...
    while True:
        ...
        if L==100:
            ...
            if len(bpml) ==100:
                ...
                #print(bpm_avg,"🎗",bpml,"⭕⭕")
                cv2.putText(frame, "HR: {0:.2f}".format(bpm_avg), (int(frame.shape[1]*0.8),int(frame.shape[0]*0.95)), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,0,0), 2)
            #print(detrended_data)
        ...
        with open("a.txt",mode = "a+") as f:
            f.write("time: {0:.4f} ".format(times[-1]) + ", HR: {0:.2f} ".format(bpm) + "time of the loop number "+ str(i) +" : " + str(time.time()-t0) + "\n")
