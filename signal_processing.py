...
class Signal_processing():
    ...
    def extract_color(self, ROIs):
        ...
        b = []
        for ROI in ROIs:
            r.append(np.mean(ROI[:, :, 0]))
        for ROI in ROIs:
            g.append(np.mean(ROI[:, :, 1]))
        for ROI in ROIs:
            b.append(np.mean(ROI[:, :, 2]))
        # b = np.mean(ROI[:,:,2])
        # return r, g, b
        output_val_sum = np.mean(g) + np.mean(r) + np.mean(b)
        output_val = output_val_sum / 3
        return output_val
    ...
    def interpolation(self, data_buffer, times):
        ...
        # print(times[0],times[-1],"🧧",even_times,"#########even times<=")
        # print("😀",times)
    ...
    def fft(self, data_buffer, fps):
        ...
        L = len(data_buffer)  # 100

        freqs = float(fps) / L * np.arange(L / 2 + 1)  # 16 / [1...51 elemnts]
